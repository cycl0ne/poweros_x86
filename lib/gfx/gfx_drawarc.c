...
void gfx_ArcAngle(pGfxBase GfxBase, pPB rp, INT32 x0, INT32 y0, INT32 rx, INT32 ry, INT32 angle1, INT32 angle2, int type)
{
	int s = angle1 / 64;
	int e = angle2 / 64;
	int x = 0, y = 0;
		...
		if ((slice->ay <= 0 && slice->by >= 0) ||
			...
			if (!dbl) {
				/* FIXME leaving in draws dot in center*/
			        ...
			        if (slice->psd->pb_FillMode != FILL_SOLID && mode) 
				  ts_drawpoint(slice->_GfxBase, slice->psd, slice->x0, slice->y0);
				else 
				  drawpoint(slice->_GfxBase, slice->psd, slice->x0, slice->y0);
				return;
			}
		}
		if (slice->psd->pb_FillMode != FILL_SOLID && mode)
		  ts_drawrow(slice->_GfxBase, slice->psd, slice->x0 + x0, slice->x0 + x1, slice->y0);
		else
		  drawrow(slice->_GfxBase,slice->psd, slice->x0 + x0, slice->x0 + x1, slice->y0);
		return;
	}
	...
	if (dbl) {
		if (!ret) {
			/* edges separate line to two parts */
		        if (slice->psd->pb_FillMode != FILL_SOLID && mode)
			  ts_drawrow(slice->_GfxBase, slice->psd, slice->x0 + x0, slice->x0 + x1, slice->y0 + y);
			else
			  drawrow(slice->_GfxBase,slice->psd, slice->x0 + x0, slice->x0 + x1, slice->y0 + y);
...
drawarcsegment(SLICE *slice, INT32 xp, INT32 yp, int drawon)
{
	...
	case ELLIPSEFILL:
	  ...
	  if (slice->psd->pb_FillMode != FILL_SOLID) {
	    ts_drawrow(slice->_GfxBase, slice->psd, slice->x0-xp, slice->x0+xp, slice->y0-yp);
	    ts_drawrow(slice->_GfxBase, slice->psd, slice->x0-xp, slice->x0+xp, slice->y0+yp);
	  }
	  else {
	    SetDash(slice->psd, &dm, &dc); /* Must turn off the dash settings because of drawrow() */
	    drawrow(slice->_GfxBase,slice->psd, slice->x0-xp, slice->x0+xp, slice->y0-yp);
	    drawrow(slice->_GfxBase,slice->psd, slice->x0-xp, slice->x0+xp, slice->y0+yp);
	...
	case ELLIPSE:
	  if (!drawon) return;
		...
		drawpoint(slice->_GfxBase,slice->psd, slice->x0 + xp, slice->y0 + yp);
		drawpoint(slice->_GfxBase,slice->psd, slice->x0 - xp, slice->y0 + yp);
		drawpoint(slice->_GfxBase,slice->psd, slice->x0 + xp, slice->y0 - yp);
		drawpoint(slice->_GfxBase,slice->psd, slice->x0 - xp, slice->y0 - yp);
		return;
	...
	case PIE:
	    ...
	    if (slice->psd->pb_FillMode == FILL_SOLID) SetDash(slice->psd, &dm, &dc);
...
static void drawarc(SLICE *slice)
{
	...
	if (slice->psd->pb_FillMode != FILL_SOLID) set_ts_origin(slice->psd, slice->x0 - rx, slice->y0 - ry);
	...
	while (dx < dy) {
		...
		if ((d > 0) || ((slice->type != PIE) && (slice->type != ELLIPSEFILL))) {
			if (slice->psd->pb_DashCount) {
				drawon = (slice->psd->pb_DashMask & (1 << bit)) ? 1 : 0;
				bit = (bit + 1) % slice->psd->pb_DashCount;
			} else
				drawon = 1;
...
void gfx_Arc(pGfxBase GfxBase, pPB psd, INT32 x0, INT32 y0, INT32 rx, INT32 ry,INT32 ax, INT32 ay, INT32 bx, INT32 by, int type)
{
	INT32	adir, bdir;
	SLICE	slice;
 ...
 */
void gfx_Ellipse(pGfxBase GfxBase, pPB psd, INT32 x, INT32 y, INT32 rx, INT32 ry, BOOL fill)
{
	SLICE	slice;
	PixMap_t	*pix = psd->pb_PixMap;
	if (rx < 0 || ry < 0) return;
