...
BLITFUNC gfx_FindConvBlit(pGfxBase GfxBase, pPB rp, int data_format, int op)
{
...
void gfx_ConversionBlit(pGfxBase GfxBase, pPB rp, pBlitParms parms)
{
	PixMap_t *psd = rp->pb_PixMap;
...
#endif
	KPrintF("GdConversionBlit: No convblit available\n");
}

/* fallback blitter wrapper for older 1,2,4 bpp subdrivers*/
static void BlitFallback(pGfxBase GfxBase, PixMap_t *psd, pBlitParms gc)
{
	KPrintF("GdBlit: No frameblit, using psd->BlitFallBack\n");
...
/* used by GdBlit*/
static BLITFUNC FindFrameBlit(PixMap_t *psd, int src_data_format, int op)
{
...
void gfx_Blit(pGfxBase GfxBase, pPB dstrp, INT32 dstx, INT32 dsty, INT32 width, INT32 height,
	pPB srcrp, INT32 srcx, INT32 srcy, int rop)
{
	BLITFUNC	frameblit;
	BlitParms_t parms;
	PixMap_t	*dstpsd = dstrp->pb_PixMap;
	PixMap_t	*srcpsd = srcrp->pb_PixMap;
...
KPrintF("Blit!!!!\n");
	...
	if (!frameblit) 
	{
		KPrintF("GdBlit: No frameblit found for op %d\n", rop);
		return;
	}
	...
	parms.fg_colorval = srcrp->pb_ForegroundRGB;	/* for mask convblit*/
	parms.bg_colorval = srcrp->pb_BackgroundRGB;
	parms.fg_pixelval = srcrp->pb_Foreground;		/* for palette mask convblit*/
	parms.bg_pixelval = srcrp->pb_Background;
	parms.usebg = srcrp->pb_useBg;
...
void gfx_StretchBlit(pGfxBase GfxBase, pPB dstrp, INT32 dx1, INT32 dy1, INT32 dx2,
	INT32 dy2, pPB srcrp, INT32 sx1, INT32 sy1, INT32 sx2,
	INT32 sy2, int rop)
{
	...
	int 		count;
	Rect *	prc;
	struct ClipRegion *clipregion = dstrp->pb_ClipRegion;
	struct PixMap *srcpsd = srcrp->pb_PixMap;
	struct PixMap *dstpsd = dstrp->pb_PixMap;
...
//	assert(dstpsd != srcpsd);
	...
	if (!dstpsd->FrameStretchBlit) {
		KPrintF("GdStretchBlit: no FrameStretchBlit (op %d)\n", rop);
		return;
	}
	...
	if (srcpsd->data_format == IF_RGBA8888 && (rop == ROP_SRC_OVER || rop == ROP_COPY)) {
		...
		if (!convblit) {
			KPrintF("GdStretchblit: no convblit for RGBA8888 src_over/copy\n");
			return;
		}
	} else
...
void gfx_ConvBlitInternal(pGfxBase GfxBase, pPB rp, pBlitParms gc, BLITFUNC convblit)
{
	INT32 dstx = gc->dstx;
	INT32 dsty = gc->dsty;
	...
	Rect *prc;
	PixMap_t *psd = rp->pb_PixMap;
	ClipRegion *clipregion = rp->pb_ClipRegion;
	...
	checksrc = ((gc->srcpsd != NULL) && (gc->srcpsd != psd));
	if (checksrc != 0)	CheckCursor(gc->srcpsd, srcx, srcy, srcx + width - 1, srcy + height - 1);
