...
static inline void drawpoint(pGfxBase GfxBase, pPB rp, INT32 x, INT32 y) { if (ClipPoint(rp, x, y)) rp->pb_PixMap->DrawPixel(rp->pb_PixMap, x, y, rp->pb_Foreground); }
 ...
 */
void gfx_Poly(pGfxBase GfxBase, pPB rp, int count, Point_t *points)
{
  INT32 firstx;
  INT32 firsty;
  BOOL didline;
  ...
  while (count-- > 1) {
	if (didline && (rp->pb_Mode == ROP_XOR)) drawpoint(GfxBase, rp, points->x, points->y);
	/* note: change to drawline*/
	Line(rp, points[0].x, points[0].y, points[1].x, points[1].y, TRUE);
	points++;
	didline = TRUE;
  }
  if (rp->pb_Mode == ROP_XOR) {
	  points--;
	  if (points->x == firstx && points->y == firsty) drawpoint(GfxBase, rp, points->x, points->y);
  }
 ...
 */
static int getPolyYBounds(Point_t *pts, int n, int *by, int *ty)
{
    Point_t *ptMin;
    int ymin, ymax;
    Point_t *ptsStart = pts;
...
static int getPolyXBounds(Point_t *pts, int n, int *bx, int *tx)
{
    Point_t *ptMin;
    int xmin, xmax;
    Point_t *ptsStart = pts;
...
void gfx_FillPoly(pGfxBase GfxBase, pPB rp, int count, Point_t *pointtable)
{
    ...
    Point_t *ptsOut, *FirstPoint;/* output buffer                 */
    ...
    ptsOut = FirstPoint	= (Point_t *)AllocVec(sizeof(Point_t) * dy, MEMF_FAST);
    ...
    while (--i >= 0) 
    {
		...
		{
		  if (rp->pb_FillMode != FILL_SOLID) 
			ts_drawrow(GfxBase, rp, ptsOut->x, ptsOut->x + e, ptsOut->y);
		   else
			 drawrow(GfxBase, rp, ptsOut->x, ptsOut->x + e, ptsOut->y);
		}
	++ptsOut;
    }
