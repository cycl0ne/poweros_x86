...
static inline void ALWAYS_INLINE frameblit_blit(pSD psd, pBlitParms gc,
	int SSZ, int SR, int SG, int SB, int SA,
	int DSZ, int DR, int DG, int DB, int DA, int PORTRAIT)
{
	INT32 gr_background = psd->background;
	int op = gc->op;
...
void frameblit_xxxa8888(pSD psd, pBlitParms gc)
{
...
static inline void ALWAYS_INLINE frameblit_stretchblit(pSD psd, pBlitParms gc,
	int SSZ, int SR, int SG, int SB, int SA,
	int DSZ, int DR, int DG, int DB, int DA, int PORTRAIT)
{
	INT32 gr_background = psd->background;
	 ...
	 * switch() within the inner loop to select the rop code.  
	 * A fast implementation of ROP_COPY is provided for speed.
	 * Both ROP_COPY and ROP_SRCOVER are enhanced to convert
	 * from 32/24/16bpp src to downsized destinations.
	 *
	 * Any other rop can be sped up by including a case for it, and
	 * calling APPLYOP with a constant op parameter.  The ROP_XOR_FGBG
	 * example shows this.  A specialized very fast example of
	 * ROP_CLEAR is also included.
	 *
	 * The SRC_OVER case must be handled seperately, as APPLYOP doesn't
...
//printf("sblit src_over\n");
		...
		break;
#if EXAMPLE
	/* sample fast implementation for ROP_CLEAR rop*/
	case ROP_CLEAR:
		while (--height >= 0)
		{
			unsigned char *d = dst;
	...
	/* sample fast implementation for ROP_XOR_FGBG rop*/
	case ROP_XOR_FGBG:
		while (--height >= 0)
		{
			register unsigned char *s = src;
			...
			while (--w >= 0)
			{
				...
				switch (DSZ) {
				case 4:
					APPLYOP(ROP_XOR_FGBG, 1, *(ADDR32), s, *(ADDR32), d, 0, 0);
					break;
				case 3:
					APPLYOP(op, 3, *(ADDR8), s, *(ADDR8), d, 1, 1);
					...
					break;
				case 2:
					APPLYOP(ROP_XOR_FGBG, 1, *(ADDR16), s, *(ADDR16), d, 0, 0);
					break;
				case 1:
					APPLYOP(ROP_XOR_FGBG, 1, *(ADDR8), s, *(ADDR8), d, 0, 0);
					break;
				}
				d += dsz;
				s += src_x_step;
...
void
frameblit_stretch_xxxa8888(pSD psd, pBlitParms gc)
{
...
void
frameblit_stretch_24bpp(pSD psd, pBlitParms gc)
{
...
void
frameblit_stretch_16bpp(pSD psd, pBlitParms gc)
{
...
void
frameblit_stretch_8bpp(pSD psd, pBlitParms gc)
{
...
void
frameblit_stretch_rgba8888_bgra8888(pSD psd, pBlitParms gc)
{
...
void
frameblit_stretch_rgba8888_bgr888(pSD psd, pBlitParms gc)
{
	/* works for src_over and copy only*/
...
void
frameblit_stretch_rgba8888_16bpp(pSD psd, pBlitParms gc)
{
	/* works for src_over and copy only*/
