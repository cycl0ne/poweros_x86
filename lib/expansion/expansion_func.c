...
#include "expansion.h"
#include "pci.h"
#define SysBase ExpBase->lib_SysBase
...
BOOL PCI_FindDevice(pExpansionBase ExpBase, UINT16 vendorId, UINT16 deviceId, PCIAddress *addrOut)
{
	...
	MemSet(&busScan, sizeof (PCIScanState), 0);

	while (PCI_ScanBus(ExpBase, &busScan)) 
	{
		KPrintF("Found: Vend: %x, Dev: %x\n",busScan.vendorId,busScan.deviceId);
...
pDosEntry exp_MakeDosNode(pExpansionBase ExpansionBase, struct ExpDosNode *parameter)
{
	return NULL;
}
