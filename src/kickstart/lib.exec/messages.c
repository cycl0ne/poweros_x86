...
static UINT32 WaitPrepare(SysBase *SysBase, Task *task)
{
	UINT32 ipl = Disable();
	if (task)
	{
...
static void WaitFinish(SysBase *SysBase, UINT32 ipl)
{
	Enable(ipl);
}

static void WaitTask(SysBase *SysBase, struct Task *Task)
{
...
UINT32 lib_Wait(SysBase *SysBase, UINT32 signalSet)
{
	...
	{
		ipl = WaitPrepare(SysBase, thisTask);
		thisTask->SigWait=signalSet;
		WaitTask(SysBase, thisTask);
		WaitFinish(SysBase, ipl);
	}
