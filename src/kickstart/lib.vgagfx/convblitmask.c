...
static inline void __attribute__ ((always_inline)) funcname(PixMap *psd, pCGfxBlitParms gc,\
...
{\
	...
	int usebg = gc->usebg;\
	UINT32 fg = gc->fg_colorval;				/* color values in MWCOLORVAL format*/\
 ...
 */
static inline void __attribute__ ((always_inline))  convblit_blend_mask_alpha_byte(PixMap * psd, pCGfxBlitParms gc,
	int DSZ, int DR, int DG, int DB, int DA, int PORTRAIT)
{
	unsigned char *src, *dst;
	int dsz, dst_pitch;
