...
#define ADDR32 UINT32*
#define ADDR16 UINT16*
#define ADDR8  UINT8*
...
{ \
	if (psd->_Update) \
		psd->_Update(psd, gc->dstx, gc->dsty, gc->width, gc->height); \
}
...
\
	case LEFT: \
		...
		psd->_Update(psd, gc->dstx, gc->dsty - gc->width + 1, gc->height, gc->width); \
		break; \
\
	case RIGHT: \
		/* adjust x,y,w,h to physical top left and w/h*/ \
		psd->_Update(psd, gc->dstx - gc->height + 1, gc->dsty, gc->height, gc->width); \
		break; \
\
	case DOWN: \
		/* adjust x,y,w,h to physical top left and w/h*/ \
		psd->_Update(psd, gc->dstx - gc->width + 1, gc->dsty - gc->height + 1, gc->width, gc->height); \
		break; \
	} \
}
...
static inline void __attribute__ ((always_inline)) frameblit_blit(PixMap *psd, pCGfxBlitParms gc,
	...
	int DSZ, int DR, int DG, int DB, int DA, int PORTRAIT)
{
	...
	if (psd->portrait == gc->srcpsd->portrait) 	FRAMEBLIT_ROTATE_COORDS(psd, gc)
	 ...
	 */
	UINT32 gr_background = 0x00; // UGH! :( We need to get the Value from the rastport :-P
	DRAWON;
	switch (op) {
	case ROP_COPY:
//printf("blit copy\n");
...
//printf("blit op %d\n", op);
		while (--height >= 0)
		{
			...
			switch (DSZ) {
			case 4:
				APPLYOP(op, width, *(UINT32*), s, *(UINT32*), d, ssz, dsz);
				break;
			case 3:
				{
					int w = width;
					while (--w >= 0)
					{
						APPLYOP(op, 3, *(UINT8*), s, *(UINT8*), d, 1, 1);
						s += ssz - 3;
						d += dsz - 3;
					}
				}
				break;
			case 2:
				APPLYOP(op, width, *(UINT16*), s, *(UINT16*), d, ssz, dsz);
				break;
			case 1:
				APPLYOP(op, width, *(UINT8*), s, *(UINT8*), d, ssz, dsz);
				break;
			}
			src += src_pitch;
			dst += dst_pitch;
...
static inline void __attribute__ ((always_inline)) frameblit_stretchblit(PixMap * psd, pCGfxBlitParms gc,
	int SSZ, int SR, int SG, int SB, int SA,
	int DSZ, int DR, int DG, int DB, int DA, int PORTRAIT)
{
	 ...
	 */
	DRAWON;
	UINT32 gr_background = 0x0; // UGGLYYY !! Same as above !
	switch (op) {
	case ROP_COPY:
//printf("sblit copy\n");
...
void
frameblit_stretch_24bpp(PixMap * psd, pCGfxBlitParms gc)
{
...
void
frameblit_stretch_16bpp(PixMap * psd, pCGfxBlitParms gc)
{
...
void
frameblit_stretch_8bpp(PixMap * psd, pCGfxBlitParms gc)
{
...
void
frameblit_stretch_rgba8888_bgra8888(PixMap * psd, pCGfxBlitParms gc)
{
...
void
frameblit_stretch_rgba8888_bgr888(PixMap * psd, pCGfxBlitParms gc)
{
	/* works for src_over and copy only*/
...
void
frameblit_stretch_rgba8888_16bpp(PixMap * psd, pCGfxBlitParms gc)
{
	/* works for src_over and copy only*/
