...
CGfxFont *cgfx_CreateFont(CoreGfxBase *CoreGfxBase, CRastPort *rp, const char *name, UINT16 height, UINT16 width, const pCGfxLogFont plogfont)
{
	...
	/* check builtin fonts first for speed*/
 	if (!height && (fontclass == LF_CLASS_ANY || fontclass == LF_CLASS_BUILTIN)) {
		 ...
		 */
		if (fontclass != LF_CLASS_ANY) DPrintF("builtin-createfont: %s,%d not found\n", fontname, height);
  	}

	/* check user builtin fonts next*/
	upf = CoreGfxBase->user_builtin_fonts;
...
{
	if (pfont->fontprocs->FSetFontSize) return pfont->fontprocs->FSetFontSize(CoreGfxBase, pfont, height, width);
	return 0;
}
...
int cgfx_SetFontRotation(CoreGfxBase *CoreGfxBase, pCGfxFont pfont, int tenthdegrees)
{
	...
	if (pfont->fontprocs->FSetFontRotation) pfont->fontprocs->FSetFontRotation(CoreGfxBase, pfont, tenthdegrees);	
	return oldrotation;
}
...
{
	if (pfont->fontprocs->FSetFontAttr) return pfont->fontprocs->FSetFontAttr(CoreGfxBase, pfont, setflags, clrflags);	
	return 0;
}
...
{
	if (pfont->fontprocs->FDestroyFont) pfont->fontprocs->FDestroyFont(CoreGfxBase, pfont);
}
...
BOOL cgfx_GetFontInfo(CoreGfxBase *CoreGfxBase, pCGfxFont pfont, pCGfxFontInfo pfontinfo)
{
	...
	return pfont->fontprocs->FGetFontInfo(CoreGfxBase, pfont, pfontinfo);
}
 ...
 */
extern CGfxFontProcs fontprocs;
...
void cgfx_Text(CoreGfxBase *CoreGfxBase, struct CRastPort *rp, pCGfxFont pfont, INT32 x, INT32 y, const void *str, int cc,UINT32 flags)
{
	...
	if (!force_uc16)	/* remove DBCS flags if not needed*/
		flags &= ~TF_DBCSMASK;
	pfont->fontprocs->FDrawText(CoreGfxBase, pfont, rp, x, y, text, cc, flags);
...
void cgfx_GetTextSize(CoreGfxBase *CoreGfxBase, pCGfxFont pfont, const void *str, int cc, INT32 *pwidth, INT32 *pheight, INT32 *pbase, UINT32 flags)
{
	...
	if (force_uc16)		/* if UC16 conversion forced, string is DBCS*/
		dbcs_gettextsize(CoreGfxBase, pfont, text, cc, flags, pwidth, pheight, pbase);
	else pfont->fontprocs->FGetTextSize(CoreGfxBase, pfont, text, cc, flags, pwidth, pheight, pbase);
