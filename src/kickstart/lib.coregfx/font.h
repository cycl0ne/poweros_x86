...
} CGfxCFont, *pCGfxCFont;

typedef struct Font*		pCGfxFont;
typedef struct FontInfo *	pCGfxFontInfo;

typedef struct {
	int			capabilities;		/* flags for font subdriver capabilities*/
	UINT32		encoding;	/* routines expect this encoding*/
	BOOL		(*FInit)(struct CoreGfxBase *CoreGfxBase, struct CRastPort rp);
	pCGfxFont	(*FCreateFont)(struct CoreGfxBase *CoreGfxBase, const char *name, INT32 height, INT32 width, int attr);
	BOOL		(*FGetFontInfo)(struct CoreGfxBase *CoreGfxBase, pCGfxFont pfont, pCGfxFontInfo pfontinfo);
	void 		(*FGetTextSize)(struct CoreGfxBase *CoreGfxBase, pCGfxFont pfont, const void *text, int cc, UINT32 flags, INT32 *pwidth, INT32 *pheight, INT32 *pbase);
	void		(*FGetTextBits)(struct CoreGfxBase *CoreGfxBase, pCGfxFont pfont, int ch, const UINT16 **retmap, INT32 *pwidth, INT32 *pheight, INT32 *pbase);
	void		(*FDestroyFont)(struct CoreGfxBase *CoreGfxBase, pCGfxFont pfont);
	void		(*FDrawText)(struct CoreGfxBase *CoreGfxBase, pCGfxFont pfont, CRastPort *rp, INT32 x, INT32 y, const void *str, int count, UINT32 flags);
	int     	(*FSetFontSize)(struct CoreGfxBase *CoreGfxBase, pCGfxFont pfont, INT32 height, INT32 width);
	void    	(*FSetFontRotation)(struct CoreGfxBase *CoreGfxBase, pCGfxFont pfont, INT16 tenthdegrees);
	int     	(*FSetFontAttr)(struct CoreGfxBase *CoreGfxBase, pCGfxFont pfont, INT16 setflags, INT16 clrflags);
	pCGfxFont	(*FDuplicate) (struct CoreGfxBase *CoreGfxBase, pCGfxFont psrcfont, INT32 height, INT32 width);
} CGfxFontProcs, *pCGfxFontProcs;


typedef struct Font {		/* common hdr for all font structures*/
	pCGfxFontProcs	fontprocs;	/* font-specific rendering routines*/
...
typedef struct CGfxCoreFont{
	/* common hdr*/
	pCGfxFontProcs	fontprocs;
	INT32			fontsize;	/* font height in pixels*/
	INT32			fontwidth;	/* font width in pixels*/
	INT32			fontrotation;
	INT32			fontattr;
	...
	pCGfxCFont	cfont;			/* builtin font data*/
} CGfxCoreFont, *pCGfxCoreFont;
