...
static inline void arch_memcpy(void *dest, const void *src, UINT32 size)
{
   asm volatile ("cld; rep movsb" : "+c" (size), "+S" (src), "+D" (dest) :: "memory");
}

static inline void arch_memset(void *dest, UINT8 value, UINT32 size)
{
   asm volatile ("cld; rep stosb" : "+c" (size), "+D" (dest) : "a" (value) : "memory");
}

static inline void arch_memcpy16(void *dest, const void *src, UINT32 size)
{
   asm volatile ("cld; rep movsw" : "+c" (size), "+S" (src), "+D" (dest) :: "memory");
}

static inline void arch_memset16(void *dest, UINT16 value, UINT32 size)
{
   asm volatile ("cld; rep stosw" : "+c" (size), "+D" (dest) : "a" (value) : "memory");
}

static inline void arch_memcpy32(void *dest, const void *src, UINT32 size)
{
   asm volatile ("cld; rep movsl" : "+c" (size), "+S" (src), "+D" (dest) :: "memory");
}

static inline void arch_memset32(void *dest, UINT32 value, UINT32 size)
{
