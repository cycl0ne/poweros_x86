...
#define SysBase ((pKbdBase)io->io_Device)->dev_SysBase
#define devBase ((pKbdBase)io->io_Device)

void (*kbd_CmdVector[])(pIOStdReq);
...
void kbd_EndCommand(UINT32 error, struct IOStdReq *io)
{
	//DPrintF("EndCommand\n");
	UINT32 ipl = Disable();
...
void (*kbd_CmdVector[])(struct IOStdReq *) = 
{
